...
def get_monthly_risk(*, start_y, month, end_year, invest_period=0):
    start_date = make_date(start_y, month, 1)
    start_date = get_next_valid_day(start_date, try_current=True)
    next_date = make_date(start_y, month, 1)

    hold_period = 10
    end_date = make_date(start_y+hold_period+invest_period, month, 1)
    start_date = get_next_valid_day(start_date, try_current=False)
    ...
    while is_before(next_date, end_date):
        next_month += 1
        if next_month > 12:
            next_month = 1
            curr_year += 1

        next_date = make_date(curr_year, next_month, 1)
        next_date = get_next_valid_day(next_date, try_current=True)
        prices.append((closes[curr_date]/closes[next_date])-1)
        curr_date = next_date
    risk = 100 * np.array(prices).std()
    return risk  
